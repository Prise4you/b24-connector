...
_SECRET_PATTERNS = [
    ...
    # Между ключевым словом и разделителем допускаются уточнения: реальные
    # выгрузки содержат «Пароль сайта: ...», «SitePass: ...», а не только
    # «пароль: ...». [^\n:=] не даёт перескочить через первый же разделитель.
    (r'(?i)(?:password|пароль|passwd|pwd|pass)[^\n:=]{0,20}[=:]\s*\S+',
                                                          "password-field"),
    ...
    # Российские паспортные данные (серия + номер) — \s+ вместо \s: реальные
    # выгрузки иногда содержат двойной пробел/таб между серией и номером.
    (r'\b\d{4}\s+\d{6}\b',                                  "passport-number"),

    # СНИЛС
    (r'\b\d{3}-\d{3}-\d{3}\s+\d{2}\b',                      "snils"),

    # Российские телефоны: +7/8/7, опциональные скобки/пробелы/дефисы между
    # группами цифр (999 123-45-67 / (999)123-45-67 / 9991234567 / 79991234567
    # и т.д. — включая слитную запись без разделителей, которая реально
    # встречается в выгрузках CRM).
    (r'(?<!\d)(?:\+7|8|7)[\s\-]?\(?\d{3}\)?[\s\-]?\d{3}[\s\-]?\d{2}[\s\-]?\d{2}(?!\d)',
                                                             "ru-phone"),

    # Email — ПДн клиента/сотрудника наравне с телефоном; сама переписка
    # не блокируется (см. README), но адрес внутри неё маскируется.
